...
Otherwise you can see the `The xxx.TypeEnsurer module implementation should be generated by Domo. Please, ensure that :domo compiler is included after the :elixir in the compilers list in the project's configuration mix.exs file.` error message on the webpage rendered by Phoenix.
...
F.e. it's not possible to define a default value in an Ecto schema like that. Same time it's possible to ensure that struct value matches the type with a call to `ensure_type/1` from `changeset/2` function of the schema.
